...
def ocr_task_producer(ocr_queue, task_queue, progress_queue, video_path, raw_subtitle_path, sub_area=None):
    """
    ...
    :param video_path
    :param raw_subtitle_path
    :param sub_area 用户手动选定的字幕区域（若设置则跳过frame_preprocess裁剪，避免坐标错位）
    """
    ...
    while True:
        try:
            ...
            # 如果读取成功
            if ret:
                # 只在用户未手动选区时才裁剪帧，避免OCR坐标与sub_area坐标系不一致
                if default_subtitle_area is not None and sub_area is None:
