...
class SubtitleExtractor:
    ...
    def __init__(self, vd_path):
        ...
        # 时间范围: (start_seconds, end_seconds) 或 None
        self.time_range = None
        # 裁剪后的临时视频路径
        self._trimmed_video_path = None
    ...
    def run(self):
        ...
        _trim_start = 0
        if self.time_range:
            start_s, end_s = self.time_range
            total_s = int(self.frame_count / self.fps) if self.fps else 0
            needs_start_trim = start_s > 0
            needs_end_trim = 0 < end_s < total_s
            if needs_start_trim or needs_end_trim:
                h_s, m_s, s_s = start_s // 3600, (start_s % 3600) // 60, start_s % 60
                h_e, m_e, s_e = end_s // 3600, (end_s % 3600) // 60, end_s % 60
                self.append_output(f"  [Khoảng quét] {h_s:02d}:{m_s:02d}:{s_s:02d} → {h_e:02d}:{m_e:02d}:{s_e:02d}")
                trimmed_path = self._trim_video(start_s, end_s)
                if trimmed_path:
                    self._trimmed_video_path = trimmed_path
                    _trim_start = start_s
                    if self.video_cap:
                        self.video_cap.release()
                    self.video_path = trimmed_path
                    self.video_cap = cv2.VideoCapture(trimmed_path)
                    self.frame_count = self.video_cap.get(cv2.CAP_PROP_FRAME_COUNT)
                    self.frame_height = int(self.video_cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
                    self.frame_width = int(self.video_cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        ...
        # 如果使用了时间范围裁剪，将字幕时间戳偏移回原始视频时间
        if _trim_start > 0 and os.path.exists(self.subtitle_output_path):
            self._offset_subtitle_timestamps(self.subtitle_output_path, _trim_start)
        # 清理临时裁剪视频
        if self._trimmed_video_path and os.path.exists(self._trimmed_video_path):
            try:
                os.remove(self._trimmed_video_path)
            except Exception:
                pass
            self._trimmed_video_path = None
       ...
       """
        ...
        if platform.system() == 'Windows':
            ...
            try:
                ...
                self.vsf_running = False
    def _trim_video(self, start_s, end_s):
        """Trim video to [start_s, end_s] using FFmpeg. Returns path to trimmed file or None."""
        try:
            from imageio_ffmpeg import get_ffmpeg_exe
            ffmpeg_path = get_ffmpeg_exe()
        except Exception:
            self.append_output('[WARNING] FFmpeg not available, time range trimming skipped')
            return None
        trimmed_path = os.path.join(self.temp_output_dir, f'_trimmed_{Path(self.video_path).stem}.mp4')
        os.makedirs(self.temp_output_dir, exist_ok=True)
        cmd = [ffmpeg_path, '-y', '-ss', str(start_s)]
        if end_s > 0:
            cmd += ['-to', str(end_s)]
        cmd += ['-i', self.video_path, '-c', 'copy', trimmed_path]
        try:
            result = subprocess.run(cmd, capture_output=True, timeout=600)
            if result.returncode != 0:
                self.append_output(f'[WARNING] FFmpeg trim failed: {result.stderr.decode(errors="ignore")[:200]}')
                return None
            return trimmed_path
        except Exception as e:
            self.append_output(f'[WARNING] FFmpeg trim error: {e}')
            return None

    def _offset_subtitle_timestamps(self, srt_path, offset_seconds):
        """Add offset_seconds to all timestamps in a SRT file."""
        try:
            subs = pysrt.open(srt_path)
            subs.shift(seconds=offset_seconds)
            subs.save(srt_path)
        except Exception as e:
            self.append_output(f'[WARNING] Failed to offset timestamps: {e}')
