...
import time
import traceback
from PySide6.QtWidgets import QWidget, QHBoxLayout, QVBoxLayout, QLabel, QTimeEdit, QSizePolicy
from PySide6.QtCore import Slot, QRect, Signal, QTime, QTimer
...
class HomeInterface(QWidget):
    ...
    def __init_widgets(self):
        ...
        self.video_display_component.play_pause_clicked.connect(self.toggle_play)
        ...
        # 时间范围选择卡
        time_range_card = CardWidget(self)
        time_range_card.setMaximumHeight(56)
        time_range_layout = QHBoxLayout()
        time_range_layout.setContentsMargins(16, 8, 16, 8)
        time_range_layout.setSpacing(8)

        range_label = QLabel(tr['SubtitleExtractorGUI']['ScanTimeRange'] + ":")
        range_label.setStyleSheet("color: #323232; font-size: 13px;")
        range_label.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        ...
        self.start_time_edit = QTimeEdit(QTime(0, 0, 0))
        self.start_time_edit.setDisplayFormat("HH:mm:ss")
        self.start_time_edit.setFixedWidth(110)
        self.start_time_edit.setStyleSheet(time_edit_style)
        self.start_time_edit.setToolTip(tr['SubtitleExtractorGUI']['StartTime'])
        ...
        time_range_layout.addWidget(arrow_label)

        self.end_time_edit = QTimeEdit(QTime(0, 0, 0))
        self.end_time_edit.setDisplayFormat("HH:mm:ss")
        self.end_time_edit.setFixedWidth(110)
        self.end_time_edit.setStyleSheet(time_edit_style)
        self.end_time_edit.setToolTip(tr['SubtitleExtractorGUI']['EndTime'])
        ...
        time_range_card.setLayout(time_range_layout)
        right_layout.addWidget(time_range_card)
    ...
    def slider_changed(self, value):
        if self.video_cap is not None and self.video_cap.isOpened():
            ...
            self.video_display_component.update_time_label(
                frame_no, self.frame_count or 0, self.fps or 25
            )
    ...
    def toggle_play(self):
        ...
        self.video_display_component.set_play_state(self._is_playing)
        if self._is_playing:
            interval = max(15, int(1000 / (self.fps or 25)))
    ...
    def _play_next_frame(self):
        ...
        if current >= max_val:
            self.play_timer.stop()
            self._is_playing = False
            self.video_display_component.set_play_state(False)
            return
        self.video_slider.setValue(current + 1)
    ...
    def on_task_selected(self, index, file_path):
        ...
        # Restore time range for this task
        time_range = self.task_list_component.get_task_option(index, TaskOptions.TIME_RANGE, None)
        self._set_time_range_ui(time_range)
    ...
    def on_time_range_changed(self):
        """Save time range to current task when user edits the time inputs."""
        ...
        self.task_list_component.update_task_option(index, TaskOptions.TIME_RANGE, (start_s, end_s))

    def _set_time_range_ui(self, time_range):
        """Update time range widgets without triggering save callbacks."""
        self.start_time_edit.blockSignals(True)
        self.end_time_edit.blockSignals(True)
        if time_range:
            start_s, end_s = time_range
            self.start_time_edit.setTime(QTime(start_s // 3600, (start_s % 3600) // 60, start_s % 60))
            self.end_time_edit.setTime(QTime(end_s // 3600, (end_s % 3600) // 60, end_s % 60))
        else:
            self.start_time_edit.setTime(QTime(0, 0, 0))
            if self.fps and self.frame_count:
                total_s = int(self.frame_count / self.fps)
    ...
    def load_video(self, video_path):
        ...
        self._set_time_range_ui(None)
        # 重置播放状态
        self._is_playing = False
        self.play_timer.stop()
        self.video_display_component.set_play_state(False)
        self.video_display_component.update_time_label(1, self.frame_count, self.fps)
        return True
